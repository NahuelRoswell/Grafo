...
public class Mapa {
	...
	private void initialize() {
		...
		btnAgregarVertice.addActionListener(new ActionListener() {
			...
			private void agregarMarcador(Coordinate coordenada, String identificador) {
				marcadores
						.add(new MapMarkerDot(identificador, new Coordinate(coordenada.getLat(), coordenada.getLon())));
		...
		btnAgregarArista.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				agregarLineaArista(inicio, destino, mapa, Color.CYAN);
			}
		});
			...
			private void buscarSolucion(int inicio, int destino, int peajes) {
				...
				if (resultado.length() > 1)
					for (int i = 1; i < resultado.length(); i++) {
						...
						agregarLineaArista(a, b, mapa, Color.red);
					}

				// agregarLineaArista();

				JOptionPane ventanaResultado = new JOptionPane();
				JOptionPane.showMessageDialog(null, resultado, "Solucion", JOptionPane.PLAIN_MESSAGE);
	...
	private void agregarLineaArista(int inicio, int destino, JMapViewer mapa, Color color) {
